...
1. Import transactions from CSV first.
2. Categorize transactions into household budget categories.
3. Compare spending to a monthly budget.
...
- Production-grade fintech compliance.
- Complex forecasting.

Start with CSV. If you try to add bank OAuth, PSD2/Open Banking, Plaid/Tink, BigQuery, a mobile app, and marriage counseling in week one, you are not building an MVP. You are building a museum of unfinished ambition.
...
ADK Budget Coach Agent
  |
  |-- Tool: load_transactions(csv_path)
  ...
  |
  v
Local Pandas / SQLite store
  |
  v
CSV transaction export + monthly budget CSV
```
